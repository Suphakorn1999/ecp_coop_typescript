...
export const getAllTeacher: RequestHandler = async (req: any, res, next) => {
  ...
  const Allteachers = await Teacher.findAll({
    ...
    attributes: ['idteacher', 'prename_teacher', 'firstname_teacher', 'lastname_teacher', 'username_teacher', 'status_teacher', 'access_rights'],
    offset: offset,
    limit: limit,
    where: {
      [Op.or]: [
...
export const updateAccess_rights: RequestHandler = async (req, res, next) => {
  if (req.body.access_rights == 1) {
    ...
    const ateacher = await Teacher.findAll({ where: { access_rights: 1 } })
    if (ateacher.length == 1) {
